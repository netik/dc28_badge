...
I_SubmitSound(void)
{
  ...
  /*
   * We know exactly how many samples to queue up during a given
   * tic so that the game video and audio stay in sync. One would
   * expect that if we queue up that exact number of samples here,
   * then they should just finish playing just in time for us to
   * queue up the next batch during the following tic.
   *
   * But that doesn't always happen with SDL. The amount of actual
   * time needed to play a given block of samples doesn't seem 
   * entirely consistent. So sometimes the audio can start lagging
   * behind the video, and the longer you play, the more it falls
   * out of sync. This behavior may vary depending on the hardware
   * platform, but we really want the audio and vide to stay
   * synchronized all the time.
   *
   * To avoid this, we check that the number of outstanding samples
   * hasn't gotten too large. If there are more than two mixbuffers
   * worth of audio outstanding, we wait for it to drain before
   * queuing up the next block.
   */

  while (SDL_GetQueuedAudioSize (audio_dev) > (sizeof (mixbuffer) * 2))
    {
    SDL_Delay (1);
    }

  if (SDL_QueueAudio (audio_dev, mixbuffer, SAMPLECOUNT*BUFMUL*2) < 0)
      I_Error ("failed to queue SDL audio\n");
