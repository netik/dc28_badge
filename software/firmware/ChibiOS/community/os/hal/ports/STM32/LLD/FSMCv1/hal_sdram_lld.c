...
void sdram_lld_selfrefresh(SDRAMDriver *sdramp) {
...
again:
    sdram_lld_wait_ready();
    SDRAMD1.sdram->SDCMR = FMCCM_SELFREFRESH | command_target;
      /* Make sure we actually get into self refresh mode. */
#if STM32_SDRAM_USE_SDRAM1
    if ((SDRAMD1.sdram->SDSR & FMC_SDSR_MODES1) != FMC_SDSR_MODES1_0)
      goto again;
#endif
#if STM32_SDRAM_USE_SDRAM2
    if ((SDRAMD1.sdram->SDSR & FMC_SDSR_MODES2) != FMC_SDSR_MODES2_0)
      goto again;
#endif
  }
}

void sdram_lld_normal(SDRAMDriver *sdramp) {
  ...
  if (sdramp->state == SDRAM_READY) {
    sdram_lld_wait_ready();
    SDRAMD1.sdram->SDCMR = FMCCM_NORMAL | command_target;
    sdram_lld_wait_ready();
  }
}
