...
/* op clock divisions */
static unsigned int opclk_divs[] = { 10, 20, 30, 40, 55, 60, 80, 120, 160 };
...
static int
wm8994RegRead (uint16_t reg, uint16_t * val)
{
	msg_t r;
	uint16_t rx;
	uint16_t tx;

	tx = __builtin_bswap16 (reg);
	rx = 0x0000;

	r = i2cMasterTransmitTimeout (&I2CD3, WM8994_I2C_ADDRESS,
	    (uint8_t *)&tx, 2, (uint8_t *)&rx, 2, 100);

	if (r != MSG_OK)
		return (-1);

	*val = __builtin_bswap16 (rx);

	return (0);
}

static int
wm8994RegWrite (uint16_t reg, uint16_t val)
{
	msg_t r;
	uint16_t tx[2];

	tx[0] = __builtin_bswap16 (reg);
	tx[1] = __builtin_bswap16 (val);

	r = i2cMasterTransmitTimeout (&I2CD3, WM8994_I2C_ADDRESS,
	    (uint8_t *)tx, 4, NULL, 0, 100);
			    ...
			    unsigned short data)
{
	...
	return (wm8994RegWrite (reg, data));
}
				    ...
				    unsigned short *data)
{
	unsigned int ret;
	(void)priv;
	ret = wm8994RegRead (reg, data);
	debug("Read Addr : 0x%04X, Data :  0x%04X\n", reg, *data);
	return (ret);
}
			    ...
			    uint channels)
{
	...
	int id = aif_id - 1;
	unsigned int i;
	int cur_val, best_val, bclk_rate, best;
				  ...
				  enum en_audio_interface aif_id)
{
	...
	if (aif_id == WM8994_AIF1) {
		...
		/*wm8994_i2c_write(priv, 0x110, 0x8100);*/
...
static struct wm8994_priv _p;

void
wm8994Init (void)
{
	wm8994_device_init(&_p);
	_wm8994_init(&_p, WM8994_AIF1, 16000, 4000000, 16, 2);
	return;
}
